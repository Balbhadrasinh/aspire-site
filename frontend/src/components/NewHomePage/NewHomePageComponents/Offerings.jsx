...
const Offerings = ({ offeringsData }) => {
  return (
    <div className="mx-auto sm:py-40 py-32 rounded-xl relative h-full w-full">
          ...
          {offeringsData?.tab &&
            offeringsData?.tab?.map((tabData, index) => (
              <SwiperSlide key={index}>
                <div className="group px-12 py-12 hover:rounded-xl hover:bg-[#0D1B2F] bg-white rounded-xl p-6 mx-4 flex flex-col cursor-pointer transition-all relative overflow-hidden">
                  <p className="text-[3rem] leading-[3.5rem] font-[400] text-[#0D1B2F] group-hover:text-white mb-4">
                    {tabData?.title}
                  </p>
                  <div className="space-y-2 mb-24 sm:mb-32">
                    {tabData?.subTitle &&
                  ...
                  <div className="relative hover:border-blue-500 w-full mx-auto rounded-full h-32 border-2 border-gray-300 bg-white mt-4 overflow-hidden">
                    <div className="flex p-10 items-center">
                      <div className="p-3 bg-white border border-[#adb5bd] rounded-full">
